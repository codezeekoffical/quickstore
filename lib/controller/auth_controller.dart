...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> loginWithSocialMedia(SocialLogInBody socialLogInBody) async {
    ...
    if (response.statusCode == 200) {
      ...
      if(_token != null && _token.isNotEmpty) {
        if(Get.find<SplashController>().configModel.customerVerification && response.body['is_phone_verified'] == 0) {
          Get.toNamed(RouteHelper.getVerificationRoute(socialLogInBody.email, _token, RouteHelper.signUp, '',null));
        }else {
          authRepo.saveUserToken(response.body['token']);
          await authRepo.updateToken();
  ...
  Future<void> registerWithSocialMedia(SocialLogInBody socialLogInBody) async {
    ...
    if (response.statusCode == 200) {
      String _token = response.body['token'];
      if(Get.find<SplashController>().configModel.customerVerification && response.body['is_phone_verified'] == 0) {
        Get.toNamed(RouteHelper.getVerificationRoute(socialLogInBody.phone, _token, RouteHelper.signUp, '',null));
      }else {
        authRepo.saveUserToken(response.body['token']);
        await authRepo.updateToken();
        Get.toNamed(RouteHelper.getAccessLocationRoute('sign-in'));
  ...
  Future<ResponseModel> verifyPhone(String phone, String token,SignUpBody signUpBody) async {
    _isLoading = true;
    update();
    Response response = await authRepo.verifyPhone(phone, _verificationCode,signUpBody);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      authRepo.saveUserToken(response.body["token"]);
      await authRepo.updateToken();
